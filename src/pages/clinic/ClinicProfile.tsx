...
export const ClinicProfile = ({
    ...
    const handleFileSelect = (field: keyof Clinic) => (e: any) => {
        const file = e.target.files?.[0];
        if (!file) return;
        const reader = new FileReader();
        reader.onload = () => {
            const result = reader.result as string;
            onClinicFieldChange(field, result);
        };
    ...
    return (
        <div className="px-6 space-y-6 pb-12">
            ...
            <div className="grid gap-2">
                <input ref={profileInputRef} type="file" accept="image/*" onChange={handleFileSelect('image')} className="hidden" />
                <input ref={coverInputRef} type="file" accept="image/*" onChange={handleFileSelect('coverImage' as keyof Clinic)} className="hidden" />
                {clinicInfo.coverImage && (
                    <div className="relative w-full h-36 rounded-xl overflow-hidden mb-2 bg-slate-100">
                        <img src={clinicInfo.coverImage} alt="cover" className="w-full h-full object-cover" />
                        <button type="button" onClick={() => { coverInputRef.current?.click(); }} className="absolute right-3 top-3 bg-white/80 p-2 rounded-full shadow">
                            <Pencil size={14} />
                        </button>
                    </div>
                )}
                <div className="flex justify-end items-center gap-4">
                    {clinicInfo.image && (
                        <div className="relative w-16 h-16 rounded-2xl">
                            <img src={clinicInfo.image} alt="profile" className="w-full h-full object-cover rounded-2xl" />
                            <button type="button" onClick={() => { profileInputRef.current?.click(); }} className="absolute right-0 bottom-0 -translate-y-1/3 translate-x-1/3 bg-white p-1 rounded-full shadow">
                                <Pencil size={12} />
                            </button>
                        </div>
                    )}
                </div>
                <p className="text-xs text-slate-500 text-end pr-4">Profile</p>
